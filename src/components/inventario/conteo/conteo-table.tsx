...
import { getConteoColumns } from "./conteo-columns";
...
export function ConteoTable({
  ...
  const columns = useMemo(
    () => getConteoColumns({ onCountChange }),
    [onCountChange]
  );
  ...
  const filteredData = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      if (filterType === "por_vencer")
        return item.estadoVencimiento === "por_vencer" || item.estadoVencimiento === "vencido";

      return true;
    });
  }, [items, globalFilter, filterType]);
          ...
          {table.getRowModel().rows.length === 0 ? (
            <tr>
              <td colSpan={columns.length} className="py-12 text-center text-slate-500 font-sans">
                <ClipboardCheck className="size-8 mx-auto stroke-[1.2] opacity-30 text-slate-400 mb-2" />
                <p className="text-sm font-semibold text-slate-400">
                  No se encontraron productos para los filtros seleccionados
                </p>
                <p className="text-xs text-slate-600 mt-0.5">
                  Prueba cambiando el término de búsqueda o el tipo de filtro.
                </p>
              </td>
            </tr>
          ) : (
            table.getRowModel().rows.map((row) => (
              <tr key={row.id} className="hover:bg-slate-900/40 transition-colors">
                {row.getVisibleCells().map((cell) => (
                  <td
                    key={cell.id}
                    className="py-2.5 px-4 border-r border-slate-800/60 last:border-r-0 align-middle"
                  >
